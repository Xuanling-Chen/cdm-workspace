 ...
 */
@RosettaDataRule("AutomaticEarlyTerminationfallbackAET")
public class AutomaticEarlyTerminationfallbackAET implements Validator<AutomaticEarlyTermination> {
	
	private static final String NAME = "AutomaticEarlyTerminationfallbackAET";
	private static final String DEFINITION = "if fallbackAET = True then partyElection->isApplicable all = False";
	...
	private ComparisonResult ruleIsApplicable(AutomaticEarlyTermination automaticEarlyTermination) {
		try {
			return areEqual(MapperS.of(automaticEarlyTermination).<Boolean>map("getFallbackAET", _automaticEarlyTermination -> _automaticEarlyTermination.getFallbackAET()), MapperS.of(Boolean.valueOf(true)), CardinalityOperator.All);
		}
