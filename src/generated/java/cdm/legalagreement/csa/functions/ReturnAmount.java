...
public abstract class ReturnAmount implements RosettaFunction {
	...
	* @param disputedReturnAmount Paragraph 5. If a party (a &#39;Disputing Party&#39;) disputes the the Calculation Agent (IM)’s calculation of a Return Amount (IM).
	...
	*/
	public Money evaluate(List<? extends PostedCreditSupportItem> postedCreditSupportItems, Money priorDeliveryAmountAdjustment, Money priorReturnAmountAdjustment, Money disputedTransferredPostedCreditSupportAmount, Money marginAmount, Money threshold, MarginApproachEnum marginApproach, Money marginAmountIA, Money minimumTransferAmount, CollateralRounding rounding, Money disputedReturnAmount, String baseCurrency) {
		// pre-conditions
			...
			assert
				areEqual(MapperS.of(baseCurrency), MapperS.of(minimumTransferAmount).<UnitType>map("getUnitOfAmount", _measureBase -> _measureBase.getUnitOfAmount()).<FieldWithMetaString>map("getCurrency", _unitType -> _unitType.getCurrency()).<String>map("getValue", _f->_f.getValue()), CardinalityOperator.All).and(areEqual(MapperS.of(baseCurrency), MapperS.of(disputedReturnAmount).<UnitType>map("getUnitOfAmount", _measureBase -> _measureBase.getUnitOfAmount()).<FieldWithMetaString>map("getCurrency", _unitType -> _unitType.getCurrency()).<String>map("getValue", _f->_f.getValue()), CardinalityOperator.All)).get()
				: "";
		
		Money.MoneyBuilder resultHolder = doEvaluate(postedCreditSupportItems, priorDeliveryAmountAdjustment, priorReturnAmountAdjustment, disputedTransferredPostedCreditSupportAmount, marginAmount, threshold, marginApproach, marginAmountIA, minimumTransferAmount, rounding, disputedReturnAmount, baseCurrency);
		Money.MoneyBuilder result = assignOutput(resultHolder, postedCreditSupportItems, priorDeliveryAmountAdjustment, priorReturnAmountAdjustment, disputedTransferredPostedCreditSupportAmount, marginAmount, threshold, marginApproach, marginAmountIA, minimumTransferAmount, rounding, disputedReturnAmount, baseCurrency);
	...
	private Money.MoneyBuilder assignOutput(Money.MoneyBuilder result, List<? extends PostedCreditSupportItem> postedCreditSupportItems, Money priorDeliveryAmountAdjustment, Money priorReturnAmountAdjustment, Money disputedTransferredPostedCreditSupportAmount, Money marginAmount, Money threshold, MarginApproachEnum marginApproach, Money marginAmountIA, Money minimumTransferAmount, CollateralRounding rounding, Money disputedReturnAmount, String baseCurrency) {
		result
			...
			if (greaterThanEquals(MapperS.of(undisputedReturnAmount(postedCreditSupportItems, priorDeliveryAmountAdjustment, priorReturnAmountAdjustment, disputedTransferredPostedCreditSupportAmount, marginAmount, threshold, marginApproach, marginAmountIA, minimumTransferAmount, rounding, disputedReturnAmount, baseCurrency).get()), MapperS.of(minimumTransferAmount).<BigDecimal>map("getAmount", _measureBase -> _measureBase.getAmount()), CardinalityOperator.All).get()) {
						return MapperS.of(roundToNearest.evaluate(MapperS.of(undisputedReturnAmount(postedCreditSupportItems, priorDeliveryAmountAdjustment, priorReturnAmountAdjustment, disputedTransferredPostedCreditSupportAmount, marginAmount, threshold, marginApproach, marginAmountIA, minimumTransferAmount, rounding, disputedReturnAmount, baseCurrency).get()).get(), MapperS.of(rounding).<BigDecimal>map("getReturnAmount", _collateralRounding -> _collateralRounding.getReturnAmount()).get(), MapperS.of(RoundingModeEnum.DOWN).get()));
					}
			else {
	...
	protected Mapper<BigDecimal> returnAmount(List<? extends PostedCreditSupportItem> postedCreditSupportItems, Money priorDeliveryAmountAdjustment, Money priorReturnAmountAdjustment, Money disputedTransferredPostedCreditSupportAmount, Money marginAmount, Money threshold, MarginApproachEnum marginApproach, Money marginAmountIA, Money minimumTransferAmount, CollateralRounding rounding, Money disputedReturnAmount, String baseCurrency) {
		return MapperS.of(max.evaluate(MapperMaths.<BigDecimal, BigDecimal, BigDecimal>subtract(MapperS.of(undisputedAdjustedPostedCreditSupportAmount(postedCreditSupportItems, priorDeliveryAmountAdjustment, priorReturnAmountAdjustment, disputedTransferredPostedCreditSupportAmount, marginAmount, threshold, marginApproach, marginAmountIA, minimumTransferAmount, rounding, disputedReturnAmount, baseCurrency).get()).<BigDecimal>map("getAmount", _measureBase -> _measureBase.getAmount()), MapperS.of(creditSupportAmount(postedCreditSupportItems, priorDeliveryAmountAdjustment, priorReturnAmountAdjustment, disputedTransferredPostedCreditSupportAmount, marginAmount, threshold, marginApproach, marginAmountIA, minimumTransferAmount, rounding, disputedReturnAmount, baseCurrency).get()).<BigDecimal>map("getAmount", _measureBase -> _measureBase.getAmount())).get(), MapperS.of(BigDecimal.valueOf(0.0)).get()));
	}
	
	protected Mapper<BigDecimal> undisputedReturnAmount(List<? extends PostedCreditSupportItem> postedCreditSupportItems, Money priorDeliveryAmountAdjustment, Money priorReturnAmountAdjustment, Money disputedTransferredPostedCreditSupportAmount, Money marginAmount, Money threshold, MarginApproachEnum marginApproach, Money marginAmountIA, Money minimumTransferAmount, CollateralRounding rounding, Money disputedReturnAmount, String baseCurrency) {
		return MapperS.of(max.evaluate(MapperMaths.<BigDecimal, BigDecimal, BigDecimal>subtract(MapperS.of(returnAmount(postedCreditSupportItems, priorDeliveryAmountAdjustment, priorReturnAmountAdjustment, disputedTransferredPostedCreditSupportAmount, marginAmount, threshold, marginApproach, marginAmountIA, minimumTransferAmount, rounding, disputedReturnAmount, baseCurrency).get()), MapperS.of(disputedReturnAmount).<BigDecimal>map("getAmount", _measureBase -> _measureBase.getAmount())).get(), MapperS.of(BigDecimal.valueOf(0.0)).get()));
	}
	public static final class ReturnAmountDefault extends ReturnAmount {
		@Override
