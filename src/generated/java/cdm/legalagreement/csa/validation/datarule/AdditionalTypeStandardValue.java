...
public class AdditionalTypeStandardValue implements Validator<AdditionalType> {
	
	private static final String NAME = "AdditionalTypeStandardValue";
	private static final String DEFINITION = "if standardValue <> AdditionalTypeEnum -> Other then customValue is absent";
	...
	private ComparisonResult ruleIsApplicable(AdditionalType additionalType) {
		try {
			return notEqual(MapperS.of(additionalType).<AdditionalTypeEnum>map("getStandardValue", _additionalType -> _additionalType.getStandardValue()), MapperS.of(AdditionalTypeEnum.OTHER), CardinalityOperator.Any);
		}
