...
public class BespokeCalculationTimeBespokeCalculationTimeTerms implements Validator<BespokeCalculationTime> {
	
	private static final String NAME = "BespokeCalculationTimeBespokeCalculationTimeTerms";
	private static final String DEFINITION = "if asCalculationAgent = False then bespokeCalculationTimeTerms exists";
	...
	private ComparisonResult ruleIsApplicable(BespokeCalculationTime bespokeCalculationTime) {
		try {
			return areEqual(MapperS.of(bespokeCalculationTime).<Boolean>map("getAsCalculationAgent", _bespokeCalculationTime -> _bespokeCalculationTime.getAsCalculationAgent()), MapperS.of(Boolean.valueOf(false)), CardinalityOperator.All);
		}
