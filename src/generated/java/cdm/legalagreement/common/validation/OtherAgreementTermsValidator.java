	...
	public ValidationResult<OtherAgreementTerms> validate(RosettaPath path, OtherAgreementTerms o) {
		String error = 
			Lists.<ComparisonResult>newArrayList(
				...
				checkCardinality("legalDocument", o.getLegalDocument()!=null ? 1 : 0, 0, 1)
			).stream().filter(res -> !res.get()).map(res -> res.getError()).collect(joining("; "));
		
		if (!isNullOrEmpty(error)) {
			return failure("OtherAgreementTerms", ValidationType.MODEL_INSTANCE, o.getClass().getSimpleName(), path, "", error);
		}
		return success("OtherAgreementTerms", ValidationType.MODEL_INSTANCE, o.getClass().getSimpleName(), path, "");
	}
