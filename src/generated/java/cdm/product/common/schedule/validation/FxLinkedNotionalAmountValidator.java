	...
	public ValidationResult<FxLinkedNotionalAmount> validate(RosettaPath path, FxLinkedNotionalAmount o) {
		String error = 
			Lists.<ComparisonResult>newArrayList(
				...
				checkCardinality("adjustedFxSpotFixingDate", o.getAdjustedFxSpotFixingDate()!=null ? 1 : 0, 0, 1),
				checkCardinality("observedFxSpotRate", o.getObservedFxSpotRate()!=null ? 1 : 0, 0, 1),
				checkCardinality("notionalAmount", o.getNotionalAmount()!=null ? 1 : 0, 0, 1)
			).stream().filter(res -> !res.get()).map(res -> res.getError()).collect(joining("; "));
		
		if (!isNullOrEmpty(error)) {
			return failure("FxLinkedNotionalAmount", ValidationType.MODEL_INSTANCE, o.getClass().getSimpleName(), path, "", error);
		}
		return success("FxLinkedNotionalAmount", ValidationType.MODEL_INSTANCE, o.getClass().getSimpleName(), path, "");
	}
