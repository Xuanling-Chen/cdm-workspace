...
public class ResetDatesNonWeeklyPeriod implements Validator<ResetDates> {
	
	private static final String NAME = "ResetDatesNonWeeklyPeriod";
	private static final String DEFINITION = "if resetFrequency -> period <> PeriodExtendedEnum -> W then resetFrequency -> weeklyRollConvention is absent";
	...
	private ComparisonResult ruleIsApplicable(ResetDates resetDates) {
		try {
			return notEqual(MapperS.of(resetDates).<ResetFrequency>map("getResetFrequency", _resetDates -> _resetDates.getResetFrequency()).<PeriodExtendedEnum>map("getPeriod", _frequency -> _frequency.getPeriod()), MapperS.of(PeriodEnum.W), CardinalityOperator.Any);
		}
	...
	private ComparisonResult evaluateThenExpression(ResetDates resetDates) {
		try {
			return notExists(MapperS.of(resetDates).<ResetFrequency>map("getResetFrequency", _resetDates -> _resetDates.getResetFrequency()).<WeeklyRollConventionEnum>map("getWeeklyRollConvention", _resetFrequency -> _resetFrequency.getWeeklyRollConvention()));
		}
		catch (ModelObjectValidator.ModelObjectValidationException ex) {
			return ComparisonResult.failure(ex.getErrors());
		}
