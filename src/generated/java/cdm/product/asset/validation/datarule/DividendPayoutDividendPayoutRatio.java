 ...
 */
@RosettaDataRule("DividendPayoutDividendPayoutRatio")
public class DividendPayoutDividendPayoutRatio implements Validator<DividendPayout> {
	
	private static final String NAME = "DividendPayoutDividendPayoutRatio";
	private static final String DEFINITION = "if dividendPayoutRatio exists then dividendPayoutRatio >= 0 and dividendPayoutRatio <= 1";
	...
	private ComparisonResult ruleIsApplicable(DividendPayout dividendPayout) {
		try {
			return exists(MapperS.of(dividendPayout).<BigDecimal>map("getDividendPayoutRatio", _dividendPayout -> _dividendPayout.getDividendPayoutRatio()));
		}
