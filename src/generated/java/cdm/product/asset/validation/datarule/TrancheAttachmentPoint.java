...
public class TrancheAttachmentPoint implements Validator<Tranche> {
	
	private static final String NAME = "TrancheAttachmentPoint";
	private static final String DEFINITION = "if Tranche exists then attachmentPoint >= 0.0 and attachmentPoint <= 1.0";
	...
	private ComparisonResult evaluateThenExpression(Tranche tranche) {
		try {
			return greaterThanEquals(MapperS.of(tranche).<BigDecimal>map("getAttachmentPoint", _tranche -> _tranche.getAttachmentPoint()), MapperS.of(BigDecimal.valueOf(0.0)), CardinalityOperator.All).and(lessThanEquals(MapperS.of(tranche).<BigDecimal>map("getAttachmentPoint", _tranche -> _tranche.getAttachmentPoint()), MapperS.of(BigDecimal.valueOf(1.0)), CardinalityOperator.All));
		}
		catch (ModelObjectValidator.ModelObjectValidationException ex) {
			return ComparisonResult.failure(ex.getErrors());
		}
