	...
	public ValidationResult<DividendCurrency> validate(RosettaPath path, DividendCurrency o) {
		String error = 
			Lists.<ComparisonResult>newArrayList(
				...
				checkCardinality("currencyReference", o.getCurrencyReference()!=null ? 1 : 0, 0, 1)
			).stream().filter(res -> !res.get()).map(res -> res.getError()).collect(joining("; "));
		
		if (!isNullOrEmpty(error)) {
			return failure("DividendCurrency", ValidationType.MODEL_INSTANCE, o.getClass().getSimpleName(), path, "", error);
		}
		return success("DividendCurrency", ValidationType.MODEL_INSTANCE, o.getClass().getSimpleName(), path, "");
	}
