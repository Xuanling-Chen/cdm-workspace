	...
	public ValidationResult<ReferencePair> validate(RosettaPath path, ReferencePair o) {
		String error = 
			Lists.<ComparisonResult>newArrayList(
				...
				checkCardinality("referenceObligation", o.getReferenceObligation()!=null ? 1 : 0, 0, 1),
				checkCardinality("noReferenceObligation", o.getNoReferenceObligation()!=null ? 1 : 0, 0, 1),
				checkCardinality("entityType", o.getEntityType()!=null ? 1 : 0, 1, 1)
			).stream().filter(res -> !res.get()).map(res -> res.getError()).collect(joining("; "));
		
		if (!isNullOrEmpty(error)) {
			return failure("ReferencePair", ValidationType.MODEL_INSTANCE, o.getClass().getSimpleName(), path, "", error);
		}
		return success("ReferencePair", ValidationType.MODEL_INSTANCE, o.getClass().getSimpleName(), path, "");
	}
