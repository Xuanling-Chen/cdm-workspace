...
public class PayoutPayRelativeTo implements Validator<Payout> {
	
	private static final String NAME = "PayoutPayRelativeTo";
	private static final String DEFINITION = "if interestRatePayout count = 2 and interestRatePayout -> paymentDates exists then interestRatePayout -> paymentDates -> payRelativeTo exists";
	...
	private ComparisonResult evaluateThenExpression(Payout payout) {
		try {
			return exists(MapperS.of(payout).<InterestRatePayout>mapC("getInterestRatePayout", _payout -> _payout.getInterestRatePayout()).<PaymentDates>map("getPaymentDates", _interestRatePayout -> _interestRatePayout.getPaymentDates()).<PayRelativeToEnum>map("getPayRelativeTo", _paymentDates -> _paymentDates.getPayRelativeTo()));
		}
		catch (ModelObjectValidator.ModelObjectValidationException ex) {
			return ComparisonResult.failure(ex.getErrors());
		}
