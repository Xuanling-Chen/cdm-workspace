 ...
 */
@RosettaDataRule("EarlyTerminationEventFpML_ird_41")
public class EarlyTerminationEventFpMLIrd41 implements Validator<EarlyTerminationEvent> {
	
	private static final String NAME = "EarlyTerminationEventFpML_ird_41";
	private static final String DEFINITION = "if EarlyTerminationEvent exists then adjustedCashSettlementValuationDate <= adjustedCashSettlementPaymentDate";
	...
	private ComparisonResult evaluateThenExpression(EarlyTerminationEvent earlyTerminationEvent) {
		try {
			return lessThanEquals(MapperS.of(earlyTerminationEvent).<Date>map("getAdjustedCashSettlementValuationDate", _earlyTerminationEvent -> _earlyTerminationEvent.getAdjustedCashSettlementValuationDate()), MapperS.of(earlyTerminationEvent).<Date>map("getAdjustedCashSettlementPaymentDate", _earlyTerminationEvent -> _earlyTerminationEvent.getAdjustedCashSettlementPaymentDate()), CardinalityOperator.All);
		}
		catch (ModelObjectValidator.ModelObjectValidationException ex) {
			return ComparisonResult.failure(ex.getErrors());
		}
