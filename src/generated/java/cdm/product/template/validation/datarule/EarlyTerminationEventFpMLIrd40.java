 ...
 */
@RosettaDataRule("EarlyTerminationEventFpML_ird_40")
public class EarlyTerminationEventFpMLIrd40 implements Validator<EarlyTerminationEvent> {
	
	private static final String NAME = "EarlyTerminationEventFpML_ird_40";
	private static final String DEFINITION = "if EarlyTerminationEvent exists then adjustedExerciseDate <= adjustedCashSettlementValuationDate";
	...
	private ComparisonResult evaluateThenExpression(EarlyTerminationEvent earlyTerminationEvent) {
		try {
			return lessThanEquals(MapperS.of(earlyTerminationEvent).<Date>map("getAdjustedExerciseDate", _earlyTerminationEvent -> _earlyTerminationEvent.getAdjustedExerciseDate()), MapperS.of(earlyTerminationEvent).<Date>map("getAdjustedCashSettlementValuationDate", _earlyTerminationEvent -> _earlyTerminationEvent.getAdjustedCashSettlementValuationDate()), CardinalityOperator.All);
		}
		catch (ModelObjectValidator.ModelObjectValidationException ex) {
			return ComparisonResult.failure(ex.getErrors());
		}
