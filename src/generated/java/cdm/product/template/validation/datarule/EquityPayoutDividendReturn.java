 ...
 */
@RosettaDataRule("EquityPayoutDividendReturn")
public class EquityPayoutDividendReturn implements Validator<EquityPayout> {
	
	private static final String NAME = "EquityPayoutDividendReturn";
	private static final String DEFINITION = "if returnType = ReturnTypeEnum -> Price then priceReturnTerms exists and dividendReturnTerms is absent";
	...
	private ComparisonResult ruleIsApplicable(EquityPayout equityPayout) {
		try {
			return areEqual(MapperS.of(equityPayout).<ReturnTypeEnum>map("getReturnType", _equityPayout -> _equityPayout.getReturnType()), MapperS.of(ReturnTypeEnum.PRICE), CardinalityOperator.All);
		}
	...
	private ComparisonResult evaluateThenExpression(EquityPayout equityPayout) {
		try {
			return exists(MapperS.of(equityPayout).<PriceReturnTerms>map("getPriceReturnTerms", _equityPayout -> _equityPayout.getPriceReturnTerms())).and(notExists(MapperS.of(equityPayout).<DividendReturnTerms>map("getDividendReturnTerms", _equityPayout -> _equityPayout.getDividendReturnTerms())));
		}
		catch (ModelObjectValidator.ModelObjectValidationException ex) {
			return ComparisonResult.failure(ex.getErrors());
		}
