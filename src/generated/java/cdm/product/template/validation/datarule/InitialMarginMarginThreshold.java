...
public class InitialMarginMarginThreshold implements Validator<InitialMargin> {
	
	private static final String NAME = "InitialMarginMarginThreshold";
	private static final String DEFINITION = "if marginThreshold exists then marginThreshold -> amount > 0";
	...
	private ComparisonResult ruleIsApplicable(InitialMargin initialMargin) {
		try {
			return exists(MapperS.of(initialMargin).<Money>map("getMarginThreshold", _initialMargin -> _initialMargin.getMarginThreshold()));
		}
	...
	private ComparisonResult evaluateThenExpression(InitialMargin initialMargin) {
		try {
			return greaterThan(MapperS.of(initialMargin).<Money>map("getMarginThreshold", _initialMargin -> _initialMargin.getMarginThreshold()).<BigDecimal>map("getAmount", _measureBase -> _measureBase.getAmount()), MapperS.of(Integer.valueOf(0)), CardinalityOperator.All);
		}
		catch (ModelObjectValidator.ModelObjectValidationException ex) {
			return ComparisonResult.failure(ex.getErrors());
		}
