	...
	public ValidationResult<FxSettlementRateSource> validate(RosettaPath path, FxSettlementRateSource o) {
		String error = 
			Lists.<ComparisonResult>newArrayList(
				...
				checkCardinality("nonstandardSettlementRate", o.getNonstandardSettlementRate()!=null ? 1 : 0, 0, 1)
			).stream().filter(res -> !res.get()).map(res -> res.getError()).collect(joining("; "));
		
		if (!isNullOrEmpty(error)) {
			return failure("FxSettlementRateSource", ValidationType.MODEL_INSTANCE, o.getClass().getSimpleName(), path, "", error);
		}
		return success("FxSettlementRateSource", ValidationType.MODEL_INSTANCE, o.getClass().getSimpleName(), path, "");
	}
