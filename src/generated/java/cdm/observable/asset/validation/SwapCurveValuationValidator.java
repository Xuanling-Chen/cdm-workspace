	...
	public ValidationResult<SwapCurveValuation> validate(RosettaPath path, SwapCurveValuation o) {
		String error = 
			Lists.<ComparisonResult>newArrayList(
				...
				checkCardinality("side", o.getSide()!=null ? 1 : 0, 0, 1)
			).stream().filter(res -> !res.get()).map(res -> res.getError()).collect(joining("; "));
		
		if (!isNullOrEmpty(error)) {
			return failure("SwapCurveValuation", ValidationType.MODEL_INSTANCE, o.getClass().getSimpleName(), path, "", error);
		}
		return success("SwapCurveValuation", ValidationType.MODEL_INSTANCE, o.getClass().getSimpleName(), path, "");
	}
