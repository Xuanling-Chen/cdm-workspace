	...
	public ValidationResult<CleanOrDirtyPrice> validate(RosettaPath path, CleanOrDirtyPrice o) {
		String error = 
			Lists.<ComparisonResult>newArrayList(
				checkCardinality("cleanPrice", o.getCleanPrice()!=null ? 1 : 0, 0, 1),
				checkCardinality("dirtyPrice", o.getDirtyPrice()!=null ? 1 : 0, 0, 1)
			).stream().filter(res -> !res.get()).map(res -> res.getError()).collect(joining("; "));
		
		if (!isNullOrEmpty(error)) {
			return failure("CleanOrDirtyPrice", ValidationType.MODEL_INSTANCE, o.getClass().getSimpleName(), path, "", error);
		}
		return success("CleanOrDirtyPrice", ValidationType.MODEL_INSTANCE, o.getClass().getSimpleName(), path, "");
	}
