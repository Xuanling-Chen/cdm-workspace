 ...
 */
@RosettaDataRule("PricePositiveFXRate")
public class PricePositiveFXRate implements Validator<Price> {
	
	private static final String NAME = "PricePositiveFXRate";
	private static final String DEFINITION = "if priceType = PriceTypeEnum -> ExchangeRate then amount > 0";
	...
	private ComparisonResult ruleIsApplicable(Price price) {
		try {
			return areEqual(MapperS.of(price).<PriceTypeEnum>map("getPriceType", _price -> _price.getPriceType()), MapperS.of(PriceTypeEnum.EXCHANGE_RATE), CardinalityOperator.All);
		}
