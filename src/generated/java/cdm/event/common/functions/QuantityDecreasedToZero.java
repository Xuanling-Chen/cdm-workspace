...
public abstract class QuantityDecreasedToZero implements RosettaFunction {
	...
	*/
	...
	private Boolean assignOutput(Boolean result, List<? extends QuantityChangePrimitive> quantityChangePrimitive) {
		result = areEqual(MapperS.of(listsCompare.evaluate(MapperS.of(CompareOp.GREATER).get(), MapperS.of(beforeQuantity(quantityChangePrimitive).get()).<QuantityGroup>mapC("getQuantityGroups", _quantityGroups -> _quantityGroups.getQuantityGroups()).<BigDecimal>mapC("getAmount", _quantityGroup -> _quantityGroup.getAmount()).getMulti(), null, MapperS.of(BigDecimal.valueOf(0.0)).get())), MapperS.of(Boolean.valueOf(true)), CardinalityOperator.All).and(areEqual(MapperS.of(listsCompare.evaluate(MapperS.of(CompareOp.EQUAL).get(), MapperS.of(afterQuantity(quantityChangePrimitive).get()).<QuantityGroup>mapC("getQuantityGroups", _quantityGroups -> _quantityGroups.getQuantityGroups()).<BigDecimal>mapC("getAmount", _quantityGroup -> _quantityGroup.getAmount()).getMulti(), null, MapperS.of(BigDecimal.valueOf(0.0)).get())), MapperS.of(Boolean.valueOf(true)), CardinalityOperator.All)).get();
		return result;
	}
	...
	public static final class QuantityDecreasedToZeroDefault extends QuantityDecreasedToZero {
		@Override
