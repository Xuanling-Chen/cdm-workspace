	...
	public ValidationResult<SpecialPurposeVehicleIssuerType> validate(RosettaPath path, SpecialPurposeVehicleIssuerType o) {
		String error = 
			Lists.<ComparisonResult>newArrayList(
				checkCardinality("creditRisk", o.getCreditRisk()!=null ? 1 : 0, 0, 1)
			).stream().filter(res -> !res.get()).map(res -> res.getError()).collect(joining("; "));
		
		if (!isNullOrEmpty(error)) {
			return failure("SpecialPurposeVehicleIssuerType", ValidationType.MODEL_INSTANCE, o.getClass().getSimpleName(), path, "", error);
		}
		return success("SpecialPurposeVehicleIssuerType", ValidationType.MODEL_INSTANCE, o.getClass().getSimpleName(), path, "");
	}
