...
public class AssetTypeSecuritySubType implements Validator<AssetType> {
	
	private static final String NAME = "AssetTypeSecuritySubType";
	private static final String DEFINITION = "if assetType <> AssetTypeEnum -> Security then ( securityType and debtType and equityType ) is absent";
	...
	private ComparisonResult ruleIsApplicable(AssetType assetType) {
		try {
			return notEqual(MapperS.of(assetType).<AssetTypeEnum>map("getAssetType", _assetType -> _assetType.getAssetType()), MapperS.of(AssetTypeEnum.SECURITY), CardinalityOperator.Any);
		}
	...
	private ComparisonResult evaluateThenExpression(AssetType assetType) {
		try {
			return ExpressionOperators.notExists(MapperTree.andDifferent(MapperTree.andDifferent(MapperTree.of(MapperS.of(assetType).<SecurityTypeEnum>map("getSecurityType", _assetType -> _assetType.getSecurityType())), MapperTree.of(MapperS.of(assetType).<DebtType>map("getDebtType", _assetType -> _assetType.getDebtType()))), MapperTree.of(MapperS.of(assetType).<EquityTypeEnum>map("getEquityType", _assetType -> _assetType.getEquityType()))));
		}
		catch (ModelObjectValidator.ModelObjectValidationException ex) {
			return ComparisonResult.failure(ex.getErrors());
		}
